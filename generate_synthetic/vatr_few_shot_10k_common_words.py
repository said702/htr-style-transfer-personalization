"""Generate the 10k common-word few-shot synthetic GoBo-style dataset with VATr."""
...
COMMON_WORDS_FILE = Path(__file__).resolve().parent / "common_words_10k.txt"

IMAGE_EXTS = (".png", ".jpg", ".jpeg")

STYLE_OUTPUT_DIR = "few_shot_styles_10k_common_words"
GENERATED_OUTPUT_DIR = "generated_vatr_10k_common_words"
...
def collect_style_entries_per_writer(project_root):
    """Collect real GoBo images that are used only as writer-style references."""
    entries_per_writer = {}
    ...
    for writer_id in WRITER_IDS:
        ...
        print(
            f"writer_{writer_id}: "
            f"{len(entries)} GoBo style candidates found"
        )

    return entries_per_writer


def collect_common_word_entries(common_words_file=COMMON_WORDS_FILE):
    """Read the 10k common-word vocabulary used for Big Style Transfer.

    Expected format:
        000000.png\ttext
        000001.png\tA few
    """
    common_words_file = Path(common_words_file)

    if not common_words_file.exists():
        raise FileNotFoundError(f"Common-word vocabulary file not found: {common_words_file}")

    entries = []
    used_names = set()

    with common_words_file.open("r", encoding="utf-8") as f:
        for line_idx, line in enumerate(f, start=1):
            line = line.strip()
            ...
            if "\t" in line:
                output_filename, word = line.split("\t", 1)
            else:
                parts = line.split(maxsplit=1)

                if len(parts) != 2:
                    raise ValueError(
                        f"Invalid line {line_idx} in {common_words_file}: {line}"
                    )

                output_filename, word = parts
            ...
            if not output_filename.lower().endswith(".png"):
                raise ValueError(
                    f"Expected PNG filename in line {line_idx}, got: {output_filename}"
                )

            if output_filename in used_names:
                raise ValueError(f"Duplicate output filename in common-word file: {output_filename}")
    ...
    if not entries:
        raise ValueError(f"No entries found in {common_words_file}")

    print(f"Loaded {len(entries)} common-word entries from {common_words_file}")
    return entries
...
def main():
    ...
    print("\n" + "=" * 80)
    print("Collecting GoBo style samples and common-word targets")
    print("=" * 80)

    style_entries_per_writer = collect_style_entries_per_writer(project_root)
    common_word_entries = collect_common_word_entries()

    print("\nSamples to generate per writer:")

    for writer_id in WRITER_IDS:
        entries = common_word_entries
        out_dir = project_root / GENERATED_OUTPUT_DIR / f"writer_{writer_id}"
    ...
    for writer_index, writer_id in enumerate(WRITER_IDS, start=1):
        ...
        style_entries = style_entries_per_writer[writer_id]
        entries = common_word_entries
